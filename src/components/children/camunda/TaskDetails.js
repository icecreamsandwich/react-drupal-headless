...
export default class TaskDetails extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state && this.state.task_id) {
      axios
        .post(host + '/camunda/task_details/' + this.state.task_id)
        .then(responseData => {
          this.setState({
  ...
  handleSubmit(e) {
    ...
    if (this.state.task_id) {
      axios
        .post(host + '/camunda/task/resolve/' + this.state.task_id, {
          json: json,
        })
        .then(responseData => {
        ...
        <React.Fragment>
          ...
          >
            <table className="table">
              ...
              {this.state.additionalVars && this.state.additionalVars.length > 0
                        ...
                        <tr>
                          <td>
                            <input
                              ...
                              onBlur={e => this.handleKeyChange(e, index)}
                            />
                          </td>
                          <td>
                            <input
                          ...
                          <td>
                            <button
                              onClick={e => this.removeVariable(e, index)}
                            >
                              Remove Variable
                            </button>
                          </td>
